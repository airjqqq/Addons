...
function AVRMarkerMesh:GenerateMesh()
	...
	if self.line then
		...
		local l=AVRLine:New(v2,self.lineVertex,128)
		self:AddLine(l)
	end
...
function AVRMarkerMesh:OnUpdate(threed)
	...
	if self.line then
		...
		local guid=(self.followGUID)
		local unit=(self.followUnit or (self.followPlayer and "player") or nil)
		local px,py,pz=threed:GetUnitPosition("player")
		local v=self.vertices[self.lineVertex]
		v[1]=nil
		if px~=0 or py~=0 then
			if unit then
				local ux,uy,uz=threed:GetUnitPosition(unit)
