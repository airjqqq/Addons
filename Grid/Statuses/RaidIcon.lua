...
local GridStatusRaidIcon = Grid:NewStatusModule("GridStatusRaidIcon", "AceTimer-3.0")
...
function GridStatusRaidIcon:PostInitialize()
	...
	self:RegisterStatus("raid_icon_target", L["Raid Icon Target"], {}, true)
end

function GridStatusRaidIcon:OnStatusEnable(status)
	self:Debug("OnStatusEnable", status)
	self:ScheduleRepeatingTimer("UpdateAllUnits",0.1)
...
function GridStatusRaidIcon:UpdateAllUnits(event, ...)
	...
	for guid, unit in Roster:IterateRoster() do
		...
		do
			local i = GetRaidTargetIndex(unit.."target")
			-- self:Debug(unit, i, i and settings.text[i], i and settings.icon[i])
			if i then
				self.core:SendStatusGained(guid, "raid_icon_target",
					settings.priority,
					nil,
					settings.color[i],
					settings.text[i],
				...
				self.core:SendStatusLost(guid, "raid_icon_target")
			end
		end
	end
end
