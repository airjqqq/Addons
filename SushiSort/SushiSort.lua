...
--    Uncomment the lines in this function to enable lots of debug output.
...
local function DoMoves()
    ...
    while (current ~= nil or #moves > 0) do
        Log(DETAIL_DEBUG, "Sorter Moves left: "..#moves);
        if current ~= nil then    
...
local scantt = CreateFrame("GameTooltip", "SushiSort_Tooltip", UIParent, "GameTooltipTemplate");
    ...
    for i=1, items, 1 do
        ...
        if item.id ~= nil then
            ...
            if item.id == 82800 then -- GetItemInfo returns a bogus name for caged pets
                local _, _, speciesID, level, breedQuality, maxHealth, power, speed, name = scantt:SetBagItem(bagID, i);
...
local function CreateBagFromTab(tab)
    ...
    for i=1, items, 1 do
        ...
        if item.id ~= nil then
            ...
            item.name, _, item.quality, _, _, item.class, item.subclass, _, item.type, _, item.price = GetItemInfo(item.id);
            if item.id == 82800 then -- GetItemInfo returns a bogus name for caged pets
                local speciesID, level, breedQuality, maxHealth, power, speed, name = scantt:SetGuildBankItem(tab, i);
                item.name = strjoin("#",name,level,breedQuality,maxHealth,power,speed);
            end
        end
        table.insert(bag, item);
...
function SUSHISORT_MainFrame_OnLoad(self)
    ...
    L = 
    {
        ...
        SUSHISORT_OPTIONSPANEL_CREDITS2 = "We don't have a website anymore.",
...
function SUSHISORT_MainFrame_OnEvent(self, event, ...)
    ...
    if event == "BANKFRAME_OPENED" then 
        ...
        local bankframe = _G["BagnonFramebank"] or _G["BankFrame"];
        if bankframe and bankSort == false then
            Log(DETAIL_EVENT, "bankSort == false");
            bankSort = true;
            CreateSortCheck("BankFrameSortCheck", bankframe, 68, -10)
            CreateSortButton("BankFrameSortButton", bankframe, 125, -35, SUSHISORT_BankSortButton);
        end
        if SUSHISORT.IsEnabled == true then 
    ...
    elseif event == "GUILDBANKFRAME_OPENED" then 
        ...
        if guildSort == false then
            ...
            guildSort = true;
            local gbframe = _G["BagnonFrameguildbank"] or _G["BagnonFrameguild"] or _G["GuildBankFrame"];
            if gbframe then
                if SUSHISORT.AltLoc == true then 
                    CreateSortButton("GBankFrameSortButton", gbframe, 16, -380, SUSHISORT_GuildSortButton);
                else
                    CreateSortButton("GBankFrameSortButton", gbframe, 490, -36, SUSHISORT_GuildSortButton);
                end
