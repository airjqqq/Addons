...
function AirjUtil:NewFIFO(size)
	assert(size>0)
	return setmetatable({_index = 1, _size = 0, _maxSize = size},fifoMetatable)
end
...
do
	...
	function Cache:PLAYER_TARGET_CHANGED(...)
		...
		if guid then
			self.cache.targeted[guid]={t=GetTime()}
		end
		if lasttargetguid then
			self.cache.targeted[lasttargetguid]={t=GetTime()}
		end
		lasttargetguid = guid
	end
		...
		Cache.combatLogEventList = {
			...
			},
			{
				...
				fcn = function(localtime,timeStamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellId,spellName,spellSchool,...)
					...
					tinsert(to.array,{t=localtime,value=damage,guid=destGUID,spellId=spellId,spellName=spellName,periodic=periodic,magic=isMagic})
					to[spellId]=to[spellId] or {}
					to[spellId].array = to[spellId].array or {isArray = true}
					tinsert(to[spellId].array, {t=localtime,value=damage,guid=destGUID,periodic=periodic,magic=isMagic})
					-- to[spellId].last={t=localtime,value=damage,guid=destGUID,periodic=periodic,magic=isMagic}
					-- to[spellId][destGUID]={t=localtime,value=damage,periodic=periodic,magic=isMagic}
					...
					tinsert(by.array,{t=localtime,value=damage,guid=sourceGUID,spellId=spellId,spellName=spellName,periodic=periodic,magic=isMagic})
					by[spellId]=by[spellId] or {}
					by[spellId].array = by[spellId].array or {isArray = true}
					tinsert(by[spellId].array, {t=localtime,value=damage,guid=sourceGUID,periodic=periodic,magic=isMagic})
					-- by[spellId].last={t=localtime,value=damage,guid=sourceGUID,periodic=periodic,magic=isMagic}
					-- by[spellId][sourceGUID]={t=localtime,value=damage,periodic=periodic,magic=isMagic}
			...
			},
			{
				...
				fcn = function(localtime,timeStamp,event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellId,spellName,spellSchool,auraType)
					...
					if ot == "Creature" then
						if not AirjUtil:IsFIFO(self.cache.died) then
							self.cache.died = AirjUtil:NewFIFO(1000)
						end
...
do
	...
	function Cache:GetBuffs(guid,unit,spellKeys,mine)
		...
		-- self.cache.buffs[guid] = self.cache.buffs[guid] or {}
	...
	function Cache:GetDebuffs(guid,unit,spellKeys,mine)
		local toRet = {}
		if not guid then return toRet end
		-- self.cache.debuffs[guid] = self.cache.debuffs[guid] or {}
